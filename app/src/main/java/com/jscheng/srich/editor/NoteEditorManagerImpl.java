...
public class NoteEditorManagerImpl {
    ...
    public void inputSize(boolean isSelected, boolean draw) {
        mOptions.setSize(isSelected);
        if (mSelectionEnd > mSelectionStart) {
            beginRetrokeAction(false);
            applySelectionWordStyle(mSelectionStart, mSelectionEnd, isSelected, Style.SubScriptSize);
            if (draw) { requestDraw(); }
            endRetrokeAction();
        }
    }

    public void inputColors(boolean isSelected, boolean draw) {
        mOptions.setColors(isSelected);
        if (mSelectionEnd > mSelectionStart) {
            beginRetrokeAction(false);
            applySelectionWordStyle(mSelectionStart, mSelectionEnd, isSelected, Style.SubScriptColor);
            if (draw) { requestDraw(); }
            endRetrokeAction();
        }
    }
