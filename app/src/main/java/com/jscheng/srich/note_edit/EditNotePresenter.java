...
public class EditNotePresenter extends IPresenter {
    ...
    public void onCreateNoteView(EditNoteView editNoteView, Context context){
        this.mView = editNoteView;
        this.isEditorBarEnable = true;
        this.isNewNote = false;
        this.mNote = null;
        mContext = context;
        this.loadNote(context);
    }

    @Override
    protected void onCreateView(@NotNull LifecycleOwner owner) {
        this.mView = (EditNoteView)owner;
        this.isEditorBarEnable = true;
        this.isNewNote = false;
        this.mNote = null;
    ...
    private void loadNote(Context context) {
        Observable.create(new ObservableOnSubscribe<Boolean>() {
            ...
            public void subscribe(ObservableEmitter emitter) {
                mNote = NoteModel.findNote(context, mNoteid);
                if (mNote == null) {
                    mNote = NoteModel.createNote(context);
                    emitter.onNext(true);
                } else {
    ...
    private void updateNote() {
        ...
        if (mContext != null){
            if (NoteModel.isNoteNull(mNote)) {
                NoteModel.deleteNote(mContext, mNote);
                if (!isNewNote) {
                    Toast.makeText(mContext, "正在删除数据", Toast.LENGTH_SHORT).show();
                }
            } else if (mNote.isDirty()) {
                NoteModel.updateNote(mContext, mNote);
                Toast.makeText(mContext, "正在保存数据", Toast.LENGTH_SHORT).show();
            }
        }else {
            if (NoteModel.isNoteNull(mNote)) {
